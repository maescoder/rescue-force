...
const vertexShader = `
  ...
  void main() {
    ...
    // Organic "breathing" heartbeat effect
    float noiseFreq = 1.5;
    float noiseAmp = 0.3;
    vec3 noisePos = vec3(pos.x * noiseFreq + uTime, pos.y * noiseFreq + uTime, pos.z * noiseFreq);
    pos += snoise(noisePos) * noiseAmp;

    // Mouse repulsion logic
    float dist = distance(pos, uMouse);
    float force = max(0.0, 1.5 - dist); // Repulsion radius
    vec3 dir = normalize(pos - uMouse);
    pos += dir * force * uHover * 2.0;
...
const fragmentShader = `
  ...
  void main() {
    ...
    // Smooth radial gradient for glow
...
export default function BreathingEcosystem() {
  ...
  const [positions, colors, sizes] = useMemo(() => {
    ...
    const baseColor = new THREE.Color("#0ea5e9"); // Vibrant amber/blue mix could be used here
    const accentColor = new THREE.Color("#10b981"); // Jade green
  ...
  useFrame((state) => {
    ...
    if (materialRef.current) {
      materialRef.current.uniforms.uTime.value = clock.getElapsedTime() * 0.5; // Heartbeat speed
      ...
      // Increase hover effect smoothly when mouse moves
